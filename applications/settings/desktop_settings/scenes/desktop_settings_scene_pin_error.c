...
void desktop_settings_scene_pin_error_on_enter(void* context) {
    ...
    if(state == SCENE_STATE_PIN_ERROR_MISMATCH) {
        desktop_view_pin_input_set_label_primary(app->pin_input_view, 14, 25, "PIN Mismatch!");
    } else if(state == SCENE_STATE_PIN_ERROR_WRONG) {
        desktop_view_pin_input_set_label_primary(app->pin_input_view, 14, 25, "Wrong PIN!");
    } else {
        furi_crash();
    }
